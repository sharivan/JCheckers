...
public class ArrayUtil {
	private static final int ZEROES_LEN = 500; // adjust
	// for
	// memory/speed
	// tradeoff
	...
	/*
	 * Compare two byte arrays returning -1, 0 or +1 if the first argument is
	 * less than, equal to, or greater than the second one. Both arguments are
	 * assumed to have the same order of byte significance. <p> When last
	 * argument is true, the comparison assumes the MSB (Most Significant Byte)
	 * is at the highest index position, and when it's false, the contrary; i.e.
	 * MSB at [0].
	 * 
	 * @since Cryptix 2.2.2 @return -1, 0 or 1 if a < b, a == b and a > b
	 * respectively.
	 */
	...
	public static byte[] redimArray(byte[] buf, int len) {
		synchronized (buf) {
			if (buf == null)
				throw new NullPointerException("redim array fail with b == null");
			if (len < 0)
				throw new ArrayIndexOutOfBoundsException("redim array fail with len < 0 (len=" + len + ")");

			byte[] b1 = new byte[len];
			System.arraycopy(buf, 0, b1, 0, Math.min(len, buf.length));

			return b1;
		}
	}

	public static <T> T[] redimArray(T[] buf, int len) {
	...
	/**
	 * Returns a byte array built from a short array. Each short is broken into
	 * 2 bytes with the short's MSB first (big-endian order).
	 * <p>
	...
	/**
	 * Returns a short array built from a byte array. Each 2 bytes form a short
	 * with the first byte as the short's MSB (big-endian order).
	 * <p>
	 * If offset and length are omitted, the whole array is used.
	 */
